...
DB_CONFIG = dict(
    ...
    dbname="ride_share", user="postgres", password="post"
)

# TODO: fill in each query to match Q6 / Q7 / Q8 from sql_assignment.md

# Q6 = Revenue by pickup city
...
# Q8 = INSERT + UPDATE for Sunita Gurung
LOYALTY_BONUS_QUERY = """
SELECT
    driver_name,
...
def print_revenue_by_city(rows):
    ...
    for city, count, revenue, avg_fare in rows:
        print(
            f"{city:<15} | rides: {count:>4} | "
            f"revenue: NPR {revenue:,.2f} | avg fare: NPR {avg_fare:,.2f}"
        )
...
def print_loyalty_bonus(rows):
    ...
    # Print each qualifying driver and their completed ride count
    for driver_name, completed_rides in rows:
        print(
            f"{driver_name:<25} | completed rides: {completed_rides:>4}"
        )

def print_outcomes_by_status(rows):
    print("\n-- Ride outcomes by status --")

    # Print each status with its ride count and average distance
    for status, count, avg_distance in rows:
        print(
            f"{status:<12} | rides: {count:>4} | "
